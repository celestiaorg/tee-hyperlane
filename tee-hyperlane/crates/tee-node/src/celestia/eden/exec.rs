...
use super::trie::{ordered_trie_root, TrieError, Witness, EMPTY_ROOT};
...
pub fn execute_block(
    ...
    for (index, raw) in input.transactions.iter().enumerate() {
        let envelope =
            TxEnvelope::decode_2718(&mut raw.as_ref()).map_err(|e| ExecError::Transaction {
                index,
                reason: e.to_string(),
            })?;
        let caller = envelope
    ...
    for (address, account) in &bundle.state {
        ...
        for (slot, value) in &account.storage {
            if !account.status.was_destroyed()
                && value.present_value == value.previous_or_original_value
            {
                continue;
            }
