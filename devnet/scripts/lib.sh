...
write_config() {
  ...
  # Base's archive: BASE_ARCHIVE as a url, else ALCHEMY_BASE_KEY from devnet/.env, else the
  # key file older hosts keep in .state. Only Base's `rpc` ever gets it.
  local base_key="${ALCHEMY_BASE_KEY:-}"
  if [ -z "${base_key}" ] && [ -f "${STATE_DIR}/alchemy-base-key" ]; then
    base_key="$(cat "${STATE_DIR}/alchemy-base-key")"
  fi
  local base_archive="${BASE_ARCHIVE:-${base_key:+https://base-sepolia.g.alchemy.com/v2/${base_key}}}"
